...
class DefaultBoxes(object):
    def __init__(self, fig_size: int, feat_size: List[int], steps: List[int], scales: List[int],
                 aspect_ratios: List[List[int]], scale_xy: float = 0.1, scale_wh: float = 0.2):
        ...
        fk: List[float] = []
        for step in steps:
            fk.append(fig_size / step)
        self.aspect_ratios = aspect_ratios

        self.default_boxes: List[List[float]] = []
...
class Encoder(object):
    ...
    def decode_batch(self, bboxes_in, scores_in, criteria: float = 0.45, max_output: int = 200):
        ...
        for bbox, prob in zip(bboxes.split(1, 0), probs.split(1, 0)):
            bbox = bbox.squeeze(0)
            prob = prob.squeeze(0)
            output.append(self.decode_single(bbox, prob, criteria, max_output, 200))
        return output

    # perform non-maximum suppression
    def decode_single(self, bboxes_in, scores_in, criteria: float, max_output: int, max_num: int = 200):
        ...
        for i, score in enumerate(scores_in.split(1, 1)):
            ...
            while score_idx_sorted.numel() > 0:
                idx = int(score_idx_sorted[-1].item())
                bboxes_sorted = bboxes[score_idx_sorted, :]
                bboxes_idx = bboxes[idx, :].unsqueeze(dim=0)
                iou_sorted = calc_iou_tensor(bboxes_sorted, bboxes_idx).squeeze()
                # we only need iou < criteria
                idx_test = iou_sorted < criteria
                if idx_test.ndim == 0:
                    idx_test = idx_test.unsqueeze(0)
                score_idx_sorted = score_idx_sorted[idx_test]
        ...
        if not bboxes_out:
            return torch.tensor(torch.jit.annotate(List[float], []), dtype=torch.float), \
                   torch.tensor(torch.jit.annotate(List[int], []), dtype=torch.long), \
                   torch.tensor(torch.jit.annotate(List[float], []), dtype=torch.float)
